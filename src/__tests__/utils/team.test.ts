...
describe("Utils/Team", () => {
	describe("Factories", () => {
		...
		test("DEVE lançar erro ao criar time com jogadores de lados diferentes", () => {
			expect(() => {
				randomTeam({
					players: [randomPlayer({ number: 1, side: Side.HOME }), randomPlayer({ number: 2, side: Side.AWAY })],
				});
		...
		test("DEVE lançar erro ao criar time com jogadores duplicados", () => {
			expect(() => {
				randomTeam({
					players: [randomPlayer({ number: 1, side: Side.HOME }), randomPlayer({ number: 1, side: Side.HOME })],
				});
			}).toThrow();
		});
	});
