...
import type { IGameClient } from "@/interfaces/client.interface.js";
...
import { sideToInt } from "@/utils/side.utils.js";
// type KnownErrorCode =
...
export class GameClient implements IGameClient {
	...
	async playAsBot(bot: IBot, onJoin?: () => void): Promise<void> {
		...
		logger.debug(`[CLIENT] Conectando no jogo do lado ${this.side} com o número ${this.number} e posição inicial ${this.initPosition.toString()}...`);
	...
	async play(callback: (inspector: GameInspector) => Promise<Order[]>, onJoin?: () => void): Promise<void> {
		...
		stream.responses.onError((error: Error & { code?: string }) => {
			logger.error(`[CLIENT] Erro na conexão com servidor do jogo, no lado ${this.side} e número ${this.number}: ${error.message}`);
		});
