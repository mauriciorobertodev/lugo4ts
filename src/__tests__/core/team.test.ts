...
import { Player, randomPlayer, randomTeam, Side, SPECS, Team } from "@/index.js";
...
describe("Core/Team", () => {
	...
	test("DEVE criar uma instância de Team corretamente", () => {
	...
	test("DEVE lançar um erro se a pontuação for negativa", () => {
		expect(() => new Team("TeamName", -1, Side.HOME, [])).toThrow(ErrTeamInvalidScore);
	});

	test("DEVE lançar um erro se um jogador tiver a side diferente do time", () => {
		const side = Side.HOME;
		const player = randomPlayer({ number: 1, side: Side.AWAY });
		expect(() => new Team("TeamName", 0, side, [player])).toThrow(ErrTeamInvalidSide);
	});

	test("DEVE lançar um erro se houver jogadores com números duplicados", () => {
		const side = Side.HOME;
	...
	test("DEVE retornar o nome do time", () => {
		const team = new Team("TeamName", 0, Side.AWAY, []);
		expect(team.getName()).toBe("TeamName");
	});

	test("DEVE retornar a pontuação do time", () => {
		const team = new Team("TeamName", 5, Side.HOME, []);
		expect(team.getScore()).toBe(5);
	});

	test("DEVE retornar o lado do time", () => {
		const team = new Team("TeamName", 0, Side.AWAY, []);
		expect(team.getSide()).toBe(Side.AWAY);
	});

	test("DEVE retornar os jogadores do time", () => {
	...
	test("DEVE retornar verdadeiro caso o jogador X esteja no time", () => {
		const side = Side.HOME;
	...
	test("DEVE retornar um erro ao tentar adicionar um jogador com a side de outro time", () => {
		const side = Side.HOME;
	...
	test("DEVE lançar erro ao tentar adicionar jogador com número duplicado", () => {
		const side = Side.HOME;
		const p1 = randomPlayer({ number: 1, side });
	...
	test("DEVE retornar um jogador aleatório do time", () => {
		const side = Side.HOME;
		const players = [randomPlayer({ number: 1, side }), randomPlayer({ number: 2, side }), randomPlayer({ number: 3, side })];
	...
	test("DEVE retornar o goleiro do time", () => {
		const side = Side.HOME;
