...
import { GoalFactory } from '../factories/goal.factory.js';
import { TeamFactory } from '../factories/team.factory.js';
...
export class GameInspector implements IGameInspector {
    ...
    getAttackGoal(): Goal {
        return this.getMyTeamSide() === Side.HOME ? GoalFactory.AWAY() : GoalFactory.HOME();
    }

    getDefenseGoal(): Goal {
        return this.getMyTeamSide() === Side.HOME ? GoalFactory.HOME() : GoalFactory.AWAY();
    }
    ...
    makeOrderKickToPoint(target: Point, speed?: number): Order {
        const direction = this.getBallPosition().directionTo(target);
        return this.makeOrderKickToDirection(direction, speed);
    }

    makeOrderMoveToDirection(direction: Vector2D, speed?: number): Order {
    ...
    makeOrderKickToDirection(direction: Vector2D, speed?: number): Order {
    ...
    makeOrderMoveToRegion(region: Region, speed?: number): Order {
        const direction = this.getMyPosition().directionTo(region.getCenter());
        return this.makeOrderMoveToDirection(direction, speed);
    }

    makeOrderKickToRegion(region: Region, speed?: number): Order {
        const direction = this.getMyPosition().directionTo(region.getCenter());
        return this.makeOrderKickToDirection(direction, speed);
    }

    makeOrderMoveToPlayer(player: Player, speed?: number): Order {
        const direction = this.getMyPosition().directionTo(player.getPosition());
        return this.makeOrderMoveToDirection(direction, speed);
    }

    makeOrderKickToPlayer(player: Player, speed?: number): Order {
        const direction = this.getMyPosition().directionTo(player.getPosition());
        return this.makeOrderKickToDirection(direction, speed);
    }
    ...
    makeOrderJumpToPoint(target: Point, speed?: number): Order {
    ...
    tryMakeOrderKickToDirection(direction: Vector2D, speed?: number): Order | null {
        if (direction.is(new Vector2D(0, 0))) return null;
    ...
    tryMakeOrderMoveToRegion(region: Region, speed?: number): Order | null {
        const direction = this.getMe().getPosition().directionTo(region.getCenter());
        return this.tryMakeOrderMoveToDirection(direction, speed);
    }

    tryMakeOrderKickToRegion(region: Region, speed?: number): Order | null {
        const direction = this.getMe().getPosition().directionTo(region.getCenter());
        return this.tryMakeOrderKickToDirection(direction, speed);
    }

    tryMakeOrderMoveToPlayer(player: Player, speed?: number): Order | null {
        const direction = this.getMe().getPosition().directionTo(player.getPosition());
        return this.tryMakeOrderMoveToDirection(direction, speed);
    }

    tryMakeOrderKickToPlayer(player: Player, speed?: number): Order | null {
        const direction = this.getMe().getPosition().directionTo(player.getPosition());
        return this.tryMakeOrderKickToDirection(direction, speed);
    }
    ...
    tryMakeOrderJumpToPoint(target: Point, speed?: number): Order | null {
        const origin = this.getMyPosition();
        const direction = origin.directionTo(target);
        if (direction.is(new Vector2D(0, 0))) return null;
        const upOrDown = direction.getY() > 0 ? new Vector2D(0, 1) : new Vector2D(0, -1);
