...
describe("Core/GameInspector", () => {
	...
	test("DEVE retornar corretamente as propriedades do game", function () {
		...
		expect(inspector.getBallRemainingTurnsInGoalZone()).toEqual(SPECS.BALL_MAX_TURNS_IN_GOAL_ZONE - snapshot.getBallTurnsInGoalZone());
	...
	test("DEVE retornar uma ordem de chute para um ponto X", function () {
		...
		if (order.action.oneofKind === "kick") {
			...
			expect(order.action.kick.velocity?.direction).toEqual(toLugoVector(inspector.getBall().getPosition().directionTo(point)));
	...
	test("DEVE retornar uma ordem de movimentação para uma região X", function () {
		...
		if (order.action.oneofKind === "move") {
			expect(order.action.move).toBeTypeOf(typeof Move);
			expect(order.action.move.velocity?.direction).toEqual(toLugoVector(inspector.getMyPosition().directionTo(region.getCenter())));
			expect(order.action.move.velocity?.speed).toEqual(SPECS.PLAYER_MAX_SPEED);
		}
	...
	test("DEVE retornar uma ordem de chute para uma região X", function () {
		...
		if (order.action.oneofKind === "kick") {
			expect(order.action.kick).toBeTypeOf(typeof Kick);
			expect(order.action.kick.velocity?.direction).toEqual(toLugoVector(inspector.getBallPosition().directionTo(region.getCenter())));
			expect(order.action.kick.velocity?.speed).toEqual(SPECS.BALL_MAX_SPEED);
		}
		...
		if (order.action.oneofKind === "kick") {
			...
			expect(() => inspector.makeOrderKickToRegion(new Region(0, 0, Side.HOME, inspector.getBallPosition(), mapper))).toThrow(ErrKickZeroDirection);
		}
	});
	...
	test("DEVE retornar uma ordem de movimentação para um player X", function () {
		...
		if (order.action.oneofKind === "move") {
			expect(order.action.move).toBeTypeOf(typeof Move);
			expect(order.action.move.velocity?.direction).toEqual(toLugoVector(inspector.getMyPosition().directionTo(otherPlayer.getPosition())));
			expect(order.action.move.velocity?.speed).toEqual(SPECS.PLAYER_MAX_SPEED);
		}
	...
	test("DEVE retornar uma ordem de chute para um player X", function () {
		...
		if (order.action.oneofKind === "kick") {
			expect(order.action.kick).toBeTypeOf(typeof Kick);
			expect(order.action.kick.velocity?.direction).toEqual(toLugoVector(inspector.getBallPosition().directionTo(otherPlayer.getPosition())));
			expect(order.action.kick.velocity?.speed).toEqual(SPECS.BALL_MAX_SPEED);
		}
	...
	test("DEVE retornar uma ordem de movimentação para um ponto X com velocidade 0", function () {
		...
		if (order.action.oneofKind === "move") {
			...
			expect(order.action.move.velocity?.direction).toEqual(toLugoVector(inspector.getMyPosition().directionTo(point)));
	...
	test("DEVE retornar uma ordem de chute para um ponto X ou null", function () {
		...
		if (order?.action.oneofKind === "kick") {
			expect(order.action.kick).toBeTypeOf(typeof Kick);
			expect(order.action.kick.velocity?.direction).toEqual(toLugoVector(inspector.getBallPosition().directionTo(point)));
			expect(order.action.kick.velocity?.speed).toEqual(SPECS.BALL_MAX_SPEED);
		}
	...
	test("DEVE retornar uma ordem de movimentação para uma região X ou null", function () {
		...
		if (order?.action.oneofKind === "move") {
			expect(order.action.move).toBeTypeOf(typeof Move);
			expect(order.action.move.velocity?.direction).toEqual(toLugoVector(myPosition.directionTo(region.getCenter())));
			expect(order.action.move.velocity?.speed).toEqual(SPECS.PLAYER_MAX_SPEED);
		}
	...
	test("DEVE retornar uma ordem de movimentação para um player X ou null", function () {
		...
		if (order?.action.oneofKind === "move") {
			expect(order.action.move).toBeTypeOf(typeof Move);
			expect(order.action.move.velocity?.direction).toEqual(toLugoVector(myPosition.directionTo(otherPlayer.getPosition())));
			expect(order.action.move.velocity?.speed).toEqual(SPECS.PLAYER_MAX_SPEED);
		}
