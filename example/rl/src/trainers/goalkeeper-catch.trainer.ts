...
import { type IGameInspector, type Order, SPECS } from "@mauricioroberto/lugo4ts";
import type { GymSession, IGymTrainer } from "@mauricioroberto/lugo4ts/gym";

// ──────────────────────────────────────────────────────────────
// Tipos
// ──────────────────────────────────────────────────────────────
type GoalkeeperInput = [number, number, number, number, number, number, number];
type GoalkeeperOutput = GKAction;
...
export class GoalkeeperCatcherTrainer implements IGymTrainer<GoalkeeperInput, GoalkeeperOutput> {
	...
	async action(inp: GoalkeeperInput): Promise<GoalkeeperOutput> {
		...
		const q = this.model.predict(tf.tensor2d([inp])) as tf.Tensor;
	...
	async evaluate(prev: IGameInspector, curr: IGameInspector): Promise<{ reward: number; done: boolean }> {
		...
		// ──────────────────────────────────────────────────────────
		const prevScore = prev.getMyTeam().getScore();
		const currScore = curr.getMyTeam().getScore();
