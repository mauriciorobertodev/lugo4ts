...
describe("Utils/GameInspector", () => {
	describe("Factories", () => {
		...
		test("DEVE lançar u erro ao gerar un GaneInspector com um player state inválido", () => {
			expect(() => randomGameInspector({ playerState: "Invalid" as PlayerState })).toThrow(ErrGameInvalidPlayerState);
		});
		...
		test("DEVE criar um GameInspector em cada estado de jogador", () => {
			...
			expect(randomGameInspectorInAsGoalKeeper({ playerState: PlayerState.HOLDING }).getMyState()).toBe(PlayerState.HOLDING);
			expect(randomGameInspectorInAsGoalKeeper({ playerState: PlayerState.DEFENDING }).getMyState()).toBe(PlayerState.DEFENDING);
			expect(randomGameInspectorInAsGoalKeeper({ playerState: PlayerState.DISPUTING }).getMyState()).toBe(PlayerState.DISPUTING);
			expect(randomGameInspectorInAsGoalKeeper({ playerState: PlayerState.SUPPORTING }).getMyState()).toBe(PlayerState.SUPPORTING);
		});

		test("DEVE lançar um erro ao criar GameInspector com estado de jogador inválido", () => {
			expect(() => randomGameInspectorInAsGoalKeeper({ playerState: "Invalid" as PlayerState })).toThrow(ErrGameInvalidPlayerState);
		});
	});
});
