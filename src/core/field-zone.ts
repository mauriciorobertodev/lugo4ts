...
import type { FieldZoneObject, IFieldZone } from "@/interfaces/field-zone.interface.js";
...
export class FieldZone implements IFieldZone {
	...
	containsRegion(region: Region): boolean {
		return region.getCol() >= this.startCol && region.getCol() <= this.endCol && region.getRow() >= this.startRow && region.getRow() <= this.endRow;
	}
	...
	clone(): FieldZone {
		return new FieldZone(this.name, this.startCol, this.endCol, this.startRow, this.endRow, this.formation.clone(), randomUUID());
	}
